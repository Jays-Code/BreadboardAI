        ...
        body: JSON.stringify({
          task: `Act as a Motion Director. Transform this scene description into a KINETIC Scene Composition JSON for Remotion.
                    ...
                    Output strict JSON with this exact structure:
                    {
                      ...
                      "sfx_triggers": [
                        { "frame": number, "url": "https://example.com/sound.mp3" }
                      ]
                    }
...
app.post("/api/visual-qa", async (req, res) => {
  ...
  try {
    for (const percent of samples) {
      ...
      try {
        (0, import_child_process.execSync)(`npx remotion still src/video/index.tsx Main "${outputPath}" --frame=${frame} --props="${propsFile}" --image-format=png --overwrite`);
