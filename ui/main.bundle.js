...
var currentIdMap = /* @__PURE__ */ new Map();
var currentNodes = [];
var nodesTotal = 0;
...
function formatNodeLabel(id) {
  const result = id.replace(/[-_]\d+$/, "").replace(/([A-Z])/g, " $1").replace(/[_-]/g, " ").replace(/\d+/g, "").trim();
...
async function renderBoardDetail(slug) {
  const app = getApp();
  if (!app) return;
  ...
  try {
    ...
    const inputs = currentNodes.filter((n) => n.type === "input");
    ...
    app.innerHTML = `
            ...
            <div class="detail-layout" style="display:block">
                ...
                <div class="guide-box">
                    <div class="guide-title">
                        <span>\u{1F44B}</span> Quick Start Guide
                    </div>
      ...
      return `
        ...
        `;
    fetch(`/api/results?slug=${slug}`).then((res) => res.json()).then((results) => {
      ...
      try {
        ...
        if (mContainer) {
          mContainer.innerHTML = `<div style="padding:1rem; color:#ffb74d; background:rgba(255,183,77,0.1); border-radius:8px;">
                        ...
                        <span style="font-size:0.8rem; font-family:monospace">${err.message}</span>
                    </div>`;
        }
      }
    } else {
    ...
    app.innerHTML = `<p style="color:red">Error loading board ${slug}: ${e.message}</p><a href="#" class="btn-secondary">Go Back</a>`;
  }
}
...
async function runBoard(slug, inputsList) {
  ...
  for (const input of inputsList) {
    ...
    if (propertyEntries.length === 0) {
      ...
      for (const [key] of propertyEntries) {
        const el = document.getElementById(`input-${input.id}-${key}`);
  ...
  timerInterval = setInterval(() => {
    const elapsed = ((Date.now() - executionStartTime) / 1e3).toFixed(1);
  ...
  const getOrCreateNodeBucket = (nodeId) => {
      ...
      if (outputs.video_url) {
        richContent += `
                    ...
                    <div style="text-align:center; margin-bottom:1rem; display:flex; justify-content:center; gap:0.5rem">
                        ...
                        <button class="btn" style="font-size:0.8rem; background:var(--accent-color)" onclick="runVisualQA('${data.runId}')">\u{1F50D} Quick QA (Frames)</button>
                        <button class="btn" style="font-size:0.8rem; background:#fb8c00" onclick="runDeepAnalysis('${data.runId}')">\u{1F3AC} Deep Analysis (Video)</button>
                    </div>
...
function getInputsForm(inputs) {
  return inputs.map((input) => {
    ...
    return propertyEntries.map(([key, prop]) => {
...
function showNodeInspector(nodeId) {
  ...
  const configEntries = Object.entries(node.configuration || {}).filter(([key]) => key !== "title" && key !== "description").map(([key, value]) => {
...
function handleRoute() {
  try {
    ...
    const app = getApp();
    if (app) app.innerHTML = `<div style="color:red; padding:2rem;">Route Error: ${e.message}</div>`;
    console.error(e);
  }
}
async function renderResultDetail(slug, resultId) {
  const app = getApp();
  if (!app) return;
  ...
  try {
    ...
    app.innerHTML = `
            ...
            <div class="panel" style="max-width:900px; margin:0 auto">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem">
                    ...
                    ${(() => {
      const hasVideoStructure = result.trace.some(
        (s) => s.data?.outputs?.video_structure || s.data?.video_structure
      );
      if (hasVideoStructure) {
        return `<button id="render-btn" class="run-button" style="padding:0.5rem 1rem; font-size:0.8rem">\u{1F3AC} Render Video</button>`;
      }
      return "";
    })()}
                </div>
                ...
                <div id="render-progress-box" style="margin-bottom:2rem; display:none; background:rgba(255,255,255,0.05); padding:1.5rem; border-radius:12px; border:1px solid rgba(255,255,255,0.1)">
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.8rem">
                        <span style="font-weight:600; font-size:0.9rem">\u{1F3AC} Rendering Video...</span>
      ...
      const finalNodesOutputs = Object.values(groups).filter((g) => g.type === "node" && g.end?.data?.outputs).map((g) => JSON.stringify(g.end.data.outputs));
        ...
        if (group.type === "special") {
          ...
          if (end) {
            ...
            Object.entries(rawData).forEach(([key, value]) => {
              if (key === "fullData" || key === "video_structure") return;
              cleanData[key] = value;
            });
    ...
    const startPolling = (rId, videoUrl) => {
    ...
    if (renderBtn) {
      renderBtn.onclick = async () => {
        const videoStruct = result.trace.find((s) => s.data?.outputs?.video_structure || s.data?.video_structure);
        if (!videoStruct) {
    ...
    app.innerHTML = `<div style="color:red; padding:2rem;">Failed to load result: ${e.message}</div>`;
  }
}
...
window.addEventListener("load", () => {
  try {
    ...
    const app = getApp();
    if (app) app.innerHTML = `<div style="color:red; padding:2rem;">Init Error: ${e.message}</div>`;
  }
});
initMermaid();
...
function showEdgePill(fromNodeId, dataSnippet) {
...
window.runVisualQA = async (runId) => {
  ...
  try {
    ...
    const assemblerNode = runData.trace.findLast((t) => t.data?.node?.id?.includes("assembler"))?.data?.outputs;
...
window.runDeepAnalysis = async (runId) => {
  const loader = document.getElementById(`analysis-loader-${runId}`);
  const content = document.getElementById(`analysis-content-${runId}`);
  const panel = document.getElementById(`analysis-results-${runId}`);
  if (!loader || !content || !panel) return;
...
function renderAnalysisReport(container, report, screenshots, title) {
  let shotsHtml = "";
  if (screenshots) {
  ...
  container.innerHTML = `
        ...
        <div style="background:rgba(0,0,0,0.2); padding:0.75rem; border-radius:6px;">
            ...
            <ul style="margin:0; padding-left:1.2rem; font-size:0.8rem; color:var(--text-secondary)">
                ${report.improvement_suggestions.map((s) => `<li style="margin-bottom:0.3rem">${s}</li>`).join("")}
            </ul>
        </div>
...
window.applyFeedback = async (runId, critique) => {
  ...
  try {
    ...
    const topicInput = document.querySelector('input[id*="topic"]');
    const critiqueInput = document.querySelector('input[id*="visual_critique"]');
